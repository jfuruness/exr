...
extern "C" {
    // Factory function to create new Announcement objects
    Announcement* Announcement_new(const char* prefix, int* as_path, int as_path_size, int timestamp,
                                   int seed_asn, bool seed_asn_present, bool roa_valid_length, bool roa_valid_length_present,
                                   int roa_origin, bool roa_origin_present, Relationships recv_relationship,
                                   bool withdraw, bool traceback_end, const char** communities, int communities_size) {
        std::vector<int> as_path_vec(as_path, as_path + as_path_size);
        std::vector<std::string> communities_vec(communities, communities + communities_size);
        std::optional<int> opt_seed_asn = seed_asn_present ? std::optional<int>(seed_asn) : std::nullopt;
        std::optional<bool> opt_roa_valid_length = roa_valid_length_present ? std::optional<bool>(roa_valid_length) : std::nullopt;
        std::optional<int> opt_roa_origin = roa_origin_present ? std::optional<int>(roa_origin) : std::nullopt;

        return new Announcement(prefix, as_path_vec, timestamp, opt_seed_asn, opt_roa_valid_length, opt_roa_origin, recv_relationship, withdraw, traceback_end, communities_vec);
    }


    void Announcement_delete(Announcement* ann) {
        delete ann;
    }
    // Other wrapper functions to expose Announcement methods...
}
